...
namespace CircusSetup
{
    public class GodotBinaryAudioStreamWAV : GodotBinaryResourceFile
    {
        ...
        public GodotBinaryAudioStreamWAV(RSD sfx, bool loop, bool SecondChannelsOnly)
        {
            byte[] SoundData = new byte[0];
            switch (sfx.CodecString)
            {
                case "XADP": // XBOX IMA ADPCM
                    SoundData = IMA_ADPCM.IMA_Decoder.Decode(sfx.Data, (int)sfx.Channels, SecondChannelsOnly);
                    break;
                case "XMA ": // XBOX 360 XMA
                    SoundData = XMA_Audio.XMA_Decoder.Decode(sfx.Data, (int)sfx.Channels, SecondChannelsOnly);
                    break;
                case "VAG ": // PS2/PSP VAG ADPCM
                    if (sfx.Channels == 4)
                        SoundData = ADPCM.ToPCMQuad(sfx.Data, sfx.Data.Length, (int)sfx.Interleave, SecondChannelsOnly);
                    if (sfx.Channels == 2)
                    ...
                    break;
                case "AT3+": // PSP ATRAC3+
                    SoundData = AT3Plus.AT3P_Decoder.Decode(sfx.Data, (int)sfx.Channels, SecondChannelsOnly);
                    break;
                default:
                    break;
            }
